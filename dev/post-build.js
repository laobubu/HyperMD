...
function patchUMD(file) {
  utils.processTextFile(file, data => {
    ...
    {
      // decide exportsAlias
      let fileModName = file
        .replace(/\.[jt]s$|^\.[\/\\]/gi, "")
        .replace(/\\/g, "/"); // => "addon/foobar"
      ...
      if (!exportsAlias) {
        ...
        if (!config.dummyComponents.some(x => minimatch(fileModName, x))) {
          console.warn(
            "[HyperMD] Not defined item in HyperMD.config: " + fileModName
          );
        }
...

    {
      ...
      for (let i = 0; i < modules.length; i++) {
        ...
        if (tmp) {
          ...
          console.warn(
            `[HyperMD][WARN] Please import "${corrected}" instead of "${mod}". Found in ${file}`
          );
        ...
        if (i < actualRefCount) {
          ...
          if (!globalName)
            console.warn(
              `[HyperMD][WARN] Module "${mod}" is NOT accessible in plain browser env! Found in ${file}`
            );
        }
      }
    }
...
(function (mod){ ${doneMark}
  /*commonjs*/  ("object"==typeof exports&&"undefined"!=typeof module) ? mod(null, exports, ${modules
    .map(x => `require("${x}")`)
    .join(", ")}) :
  /*amd*/       ("function"==typeof define&&define.amd) ? define(${JSON.stringify(
    ["require", "exports"].concat(modules)
  )}, mod) :
  /*plain env*/ mod(null, ${exportsAlias}, ${moduleGlobalNames
      .slice(0, actualRefCount)
      .join(", ")});
})(`;
...
tsconfig.include.forEach(pattern => {
  ...
  glob(pattern, (err, matches) => {
    if (err) {
      console.error(
        "[HyperMD] Error while searching output files. Pattern = " + pattern
      );
      console.error(err);
      process.exit(2);
      return;
