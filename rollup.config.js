...
const fix_core_path_in_single_file = function (sourcePath, id) {
  ...
  if (/\bcore[\\\/]/.test(id)) {
    // ./hypermd/core.js doesnot exists.
    // but ./dist/core.js does
    id = path.resolve("./hypermd/core")
  }
...
for (const id in hypermdStuff) {
  const inputFullPath = path.resolve("./hypermd/" + id + ".ts")
  var configItem = {
    input: "./hypermd/" + id + ".ts",
