      ...
      if (table != TableType.NONE) {
        if (
          ...
          (pos.line === cm.lastLine() ||
            cm.getStateAfter(pos.line + 1).hmdTable !== table)
        ) {
        ...
        for (let i = 1; i < lineTokens.length; i++) {
          ...
          if (
            token.string === "|" &&
            (!token.type || !token.type.trim().length)
          ) {
            ans += " ------- |";
            ans2 += "   |";
          }
  ...
  for (let i = 0; i < selections.length; i++) {
    ...
    if (eolState.hmdTable) {
      ...
      if (rightPipe) {
        ...
        var leftPipe = tokenSeeker.findPrev(
          isRealTableSep,
          rightPipe.i_token - 1
        );
      ...
      for (
        ;
        lineNo <= right.line && (tmp = ListRE.exec(cm.getLine(lineNo)));
        lineNo++
      ) {
        ...
        while (++lineNo <= lastLine) {
          if (
            /*corrupted */ cm.getStateAfter(lineNo).listStack.length !==
            listLevel
          ) {
            lineNo = Infinity;
            break;
    ...
    if (eolState.hmdTable) {
      ...
      if (!nextCellLeft) {
        ...
        if (
          ...
          const dummySep =
            isNormalTable && tokenSeeker.findNext(/hmd-table-sep-dummy/, 0);
      ...
      for (
        ...
        if (lineNo > firstLine && listLevel <= listStackOfPrevLine.length) {
          if (listLevel == listStackOfPrevLine.length) {
            ...
            spaces = repeatStr(
              " ",
              listStackOfPrevLine[listLevel] - tmp[0].length
            );
          }
        }
...
export function wrapTexts(
  ...
  for (let i = 0; i < selections.length; i++) {
    ...
    if (
      text.slice(0, lb_len) === leftBracket &&
      text.slice(-rb_len) === rightBracket
    ) {
...
export function createStyleToggler(
  ...
  return function(cm: cm_t) {
    ...
    for (let i = 0; i < selections.length; i++) {
      ...
      const rangeText = (replacements[i] = rangeEmpty
        ? ""
        : cm.getRange(left, right));
      ...
      if (rangeEmpty || isStyled(cm.getTokenAt(left).state)) {
        ...
        if (to.i_token === from.i_token) {
          ...
          if (isFormattingToken(to.token)) {
            cm.replaceRange(
              "",
              { line, ch: to.token.start },
              { line, ch: to.token.end }
            );
          }
          if (from.i_token !== to.i_token && isFormattingToken(from.token)) {
            cm.replaceRange(
              "",
              { line, ch: from.token.start },
              { line, ch: from.token.end }
            );
          }
        }
        continue;
...
keyMap = (CodeMirror as any).normalizeKeyMap(keyMap) as CodeMirror.KeyMap;
(CodeMirror as any).keyMap["hypermd"] = keyMap;
